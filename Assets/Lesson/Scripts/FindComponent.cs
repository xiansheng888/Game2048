...
public class FindComponent : MonoBehaviour
{
    ...
    void Awake() { DicFUNC = new Dictionary<FUNC, Action>(); if (DicFUNC.ContainsKey(FUNC.Awake)) { DicFUNC[FUNC.Awake](); } }//：唤醒事件，游戏一开始运行就执行，只执行一次。
    ...
    /// <returns></returns>
    private T GetComponentByName<T>(string objName) where T : Component
    {
        T Cmpent = null;
    ...
    /// <returns></returns>
    public Component GetComponentByName(string objName, System.Type ComponentType)
    {
        ...
        else { return this.GetComponent(ComponentType); }
