...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<FUNC>(translator.PushFUNC, translator.Get, translator.UpdateFUNC);
        ...
        public void PushFUNC(RealStatePtr L, FUNC val)
        {
            if (FUNC_TypeID == -1)
            {
			    bool is_first;
                FUNC_TypeID = getTypeId(L, typeof(FUNC), out is_first);
				
				if (FUNC_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(FUNC));
				    FUNC_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, FUNC_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, FUNC_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for FUNC ,value="+val);
            }
			
			LuaAPI.lua_getref(L, FUNC_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out FUNC val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != FUNC_TypeID)
				{
				    throw new Exception("invalid userdata for FUNC");
				}
                ...
                {
                    throw new Exception("unpack fail for FUNC");
                }
				val = (FUNC)e;
            ...
            {
                val = (FUNC)objectCasters.GetCaster(typeof(FUNC))(L, index, null);
            }
        }
		
        public void UpdateFUNC(RealStatePtr L, int index, FUNC val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for FUNC ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(FUNC[]))
			{
			    FUNC[] array = obj as FUNC[];
				translator.PushFUNC(L, array[index]);
				return true;
			}
